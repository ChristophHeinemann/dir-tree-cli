...
osLocale().then((locale) => {
	...
	i18next.use(Backend).init(
		...
		(err, t) => {
			...
			const ignoreDirectory = process.argv[3] || "node_modules";

            const targetDirectoryName = targetDirectory === "." ? t("current") : targetDirectory;

			console.log(
				t("welcome", { path: targetDirectoryName, ignore: ignoreDirectory }),
			);
